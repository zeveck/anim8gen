...
name: doc
description: >-
  Z Skills workflow adapted for Codex: Audit and fix documentation gaps: block
  library panel entries, example models, getting-started guides, block
  library, presentations, README updates. Also handles newsletter entries
  (/doc newsletter). Use when the user asks to "write a newsletter entry",
  "add to the newsletter", or "update the newsletter". Usage: doc
  [blocks|examples|newsletter|<description>]
---
