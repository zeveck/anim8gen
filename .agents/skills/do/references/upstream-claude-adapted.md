...
name: do
description: >-
  Z Skills workflow adapted for Codex: Lightweight task dispatcher for ad-hoc
  work: documentation, examples, refactoring, content updates. Supports
  scheduling with every/now/next/stop. Usage: do <description> [worktree]
  [push] [pr] [every SCHEDULE] [now] | stop | next.
---
...
**Before dispatching:** Check `agents.min_model` in `zskills-config.json` or the upstream reference config. If set,
