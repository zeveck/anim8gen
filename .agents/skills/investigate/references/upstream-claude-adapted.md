...
name: investigate
description: >-
  Z Skills workflow adapted for Codex: Deep debugging for complex bugs.
  Enforces a disciplined workflow: reproduce, trace, state root cause, fix,
  verify. The agent must PROVE it understands the root cause before writing
  any fix.
---
