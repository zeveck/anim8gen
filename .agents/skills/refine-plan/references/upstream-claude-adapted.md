...
name: refine-plan
description: >-
  Z Skills workflow adapted for Codex: Refine an in-progress plan by reviewing
  remaining phases against completed work. Dispatches adversarial reviewer and
  devil's advocate agents to find stale references, invalidated assumptions,
  and specification gaps — then refines remaining phases until convergence.
  Completed phases are NEVER modified. Appends a Drift Log and Plan Review
  section. Usage: refine-plan <plan-file> [rounds N]
---
