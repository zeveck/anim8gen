...
name: update-zskills
description: >-
  Z Skills workflow adapted for Codex: Install or update Z Skills supporting
  infrastructure (CLAUDE.md rules, hooks, scripts)
---
...
- `update-zskills` **and no existing `zskills-config.json` or the upstream reference config** —
...
non-empty, every other field in `zskills-config.json` or the upstream reference config
...
Check if `zskills-config.json` or the upstream reference config exists in the target project root (`$PROJECT_ROOT`).
...
3. Write the config file directly using the `Write` tool. Running
   ...
   Content to write to `zskills-config.json` or the upstream reference config:
   ```json
   {
...
**Run this only when** `zskills-config.json` or the upstream reference config does NOT exist AND
...
List all `~/.codex/skills/*/SKILL.md` files. For each skill:
...
2. **Detect installation state:**
   - If no `~/.codex/skills/` directory exists, or it contains zero skills
...
2. **For each add-on skill** (e.g., `add-block`, `add-example`, `model-design`):
   - If `~/.codex/skills/<name>/SKILL.md` already exists, skip (never overwrite)
   - Otherwise, copy from the add-on source directory to `~/.codex/skills/<name>/`
