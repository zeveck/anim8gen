...
name: qe-audit
description: >-
  Z Skills workflow adapted for Codex: QE audit: check recent commits for test
  coverage gaps, or bash/stress-test features to find bugs. Supports
  scheduling with every/now/next/stop. Usage: qe-audit [bash [area]] [every
  SCHEDULE] [now] | stop | next.
---
