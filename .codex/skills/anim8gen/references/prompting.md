...
```text
<style> sprite frame of <subject>, <view> view, single centered full-body subject,
pose: <frame pose>. Compact readable silhouette, consistent identity with the
reference, solid #ff00ff chroma-key background, no text, no watermark, no labels,
no UI overlay, no extra subjects, no props unless requested, no baked preview
effects.
```
